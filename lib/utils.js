...
export function delay(ms) {
    ...
    const promise = new Promise(resolve => {
        ...
        GLib.Source.set_name_by_id(id, '[floeisland] delay');
    });
    return {
        promise,
        cancel() {
...
export function timeoutMs(ms, callback) {
    ...
    GLib.Source.set_name_by_id(id, '[floeisland] timeout');
    return id;
}
...
export function runCommand(argv, {onLine = null, onExit = null} = {}) {
    ...
    try {
        ...
        logError(e, '[floeisland] failed to spawn command');
        onExit?.(-1);
        return null;
    }
    ...
    if (stdout && onLine) {
        readLines(stdout, onLine).catch(err =>
            logError(err, '[floeisland] reading command output'));
    }
...
export function ensureDir(path) {
    try {
        ...
        logError(e, '[floeisland] mkdir failed');
        return false;
    }
}
...
export function copyToClipboard(text) {
    try {
        ...
        logError(e, '[floeisland] clipboard');
    }
}
