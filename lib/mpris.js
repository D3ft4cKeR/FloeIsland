...
const FloeMprisPlayerIface = loadInterfaceXML('org.mpris.MediaPlayer2.Player');
const FloeMprisPlayerProxy = Gio.DBusProxy.makeProxyWrapper(FloeMprisPlayerIface);
...
// ---------------------------------------------------------------------------
export const FloeMprisPlayer = GObject.registerClass({
    Properties: {
...
}, class FloeMprisPlayer extends GObject.Object {
    _init(busName, {preferred = false} = {}) {
        super._init();
        this._busName = busName;
        this.preferred = preferred;
        this._canPlay = false;
        this._playerProxy = new FloeMprisPlayerProxy(Gio.DBus.session, busName,
...
// ---------------------------------------------------------------------------
export const FloeMprisWatcher = GObject.registerClass({
    Signals: {
        'player-added': {},
        'player-removed': {},
    },
}, class FloeMprisWatcher extends GObject.Object {
    ...
    _addPlayer(busName) {
        if (this._players.has(busName))
            return;
        const player = new FloeMprisPlayer(busName, {
