...
export class MprisPlayer {
    constructor(busName, {preferred = false} = {}) {
        ...
        // 异步初始化（失败时静默）
        this._playerProxy.init_async(GLib.PRIORITY_DEFAULT, null)
            .catch(() => {});
        this._rootProxy.init_async(GLib.PRIORITY_DEFAULT, null)
            .catch(() => {});
    }
...
/**
 * 播放器发现器：监听 D-Bus NameOwnerChanged，维护 MprisPlayer 列表。
 */
export class MprisWatcher {
    ...
    pick() {
        ...
        if (preferred)
            return preferred;
        return all.find(p => p.playing) ?? all[0];
    }
