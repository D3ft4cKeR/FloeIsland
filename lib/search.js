...
import {openPath} from './actions.js';
...
export class SearchController {
    constructor({entry, onResults, ext}) {
        ...
        // St.Entry 本身无 text-changed 信号，需监听其 clutter_text（与 Shell 一致）
    ...
    async _run(q) {
        ...
        // 始终搜索应用 + 文件（扩大搜索范围）；整体 try/catch 防止单点崩溃静默无结果
        let rows = [];
        try {
            ...
            if (backend !== 'apps-only') {
                const backends = backend === 'command'
                    ? ['command']
                    : backend === 'locate'
                        ? ['locate']
                        : ['tracker', 'locate'];
                for (const b of backends) {
                    if (gen !== this._gen)
                        return;
                    try {
                        const r = await this._runBackend(b, q, gen);
                        files = r;
                        if (files.length > 0)
                            break;
                    } catch (e) {
                        logError(e, `[floeisland] search backend ${b}`);
                    }
                }
            }
            if (gen !== this._gen)
                return;
            rows = [...apps, ...files].slice(0, MAX_SEARCH_RESULTS);
            // 始终在末尾添加「用浏览器搜索」选项
            rows.push({
                iconName: 'system-search-symbolic',
                title: `在浏览器中搜索「${q}」`,
                subtitle: '使用默认浏览器打开',
                run: () => openBrowserSearch(q),
            });
        } catch (e) {
            logError(e, `[floeisland] search "${q}"`);
            rows = [{
                iconName: 'system-search-symbolic',
                title: `在浏览器中搜索「${q}」`,
                subtitle: '文件/应用搜索出错，改用浏览器',
                run: () => openBrowserSearch(q),
            }];
        }
...
function searchApps(q) {
    ...
    const out = [];
    for (const raw of Gio.AppInfo.get_all()) {
        const info = raw;
        ...
        if (name.toLowerCase().includes(needle) ||
            ...
            out.push({
                ...
                // 直接用匹配到的 AppInfo 实例启动，最可靠（含 id/命令完整）
                run: () => {
                    try {
                        info.launch([], null);
                    } catch (e) {
                        logError(e, `[floeisland] launch app "${id}"`);
                    }
                },
            });
        }
...
function makeFileRow(line) {
    ...
    const isDir = file.query_file_type(Gio.FileQueryInfoFlags.NONE)
        === Gio.FileType.DIRECTORY;
    return {
        iconName: isDir ? 'folder-symbolic' : 'text-x-generic-symbolic',
...
function openBrowserSearch(query) {
    ...
    try {
        ...
        logError(e, '[floeisland] open browser search');
    }
}
