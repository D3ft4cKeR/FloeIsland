...
// 月视图格子高度：40px，6 行 = 240px + 表头/导航 ≈ 310px，
// 小于内容区高度（约 380px），保证 6 行（42 天）整月完整显示，无需滚动
const CAL_CELL_HEIGHT = 40;
    ...
    function renderMonth() {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 0; c < 7; c++) {
                ...
                // 小字：节气/节日优先，其次农历日名。
                // clamp_text + ellipsize：长节日名（如"建军节"）显示省略号，
                // 不会横向溢出盖到相邻格子
                let small = '';
                if (fests.length > 0)
                    small = fests[0];
