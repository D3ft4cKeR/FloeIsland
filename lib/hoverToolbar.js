...
// 勿扰模式：读写系统通知设置（开启勿扰 = 关闭系统横幅 → 通知不再弹出）
function getDndEnabled() {
    try {
        const s = new Gio.Settings({schema: 'org.gnome.desktop.notifications'});
        return !s.get_boolean('show-banners');
    } catch (e) {
        return false;
    }
}
function setDndEnabled(on) {
    try {
        const s = new Gio.Settings({schema: 'org.gnome.desktop.notifications'});
        s.set_boolean('show-banners', !on);
    } catch (e) {
        // ignore
    }
}
...
export function createHoverToolbar(dock, ext) {
    ...
    const toolDefs = () => [
        ...
        {
            // 勿扰模式开关：联动系统勿扰（关闭系统横幅 → 暂停通知弹出）
            icon: 'notifications-disabled-symbolic',
            label: '勿扰模式',
            get active() {
                return getDndEnabled();
            },
            toggle() {
                const next = !getDndEnabled();
                setDndEnabled(next);
                dock.showOsdInfo({
                    icon: next
                        ? 'notifications-disabled-symbolic'
                        : 'notifications-symbolic',
                    label: next ? '勿扰已开启' : '勿扰已关闭',
                    duration: 1200,
                });
            },
        },
    ...
    function build() {
        ...
        for (const def of toolDefs()) {
            ...
            btn.add_child(icon);
            btn._def = def;
            // 勿扰开关初始化时反映系统勿扰状态（激活高亮）
            if (typeof def.toggle === 'function' && def.active) {
                btn.add_style_pseudo_class('dnd-on');
                icon.icon_name = 'notifications-disabled-symbolic';
            }
            ...
            btn.connect('button-press-event', (actor, event) => {
                if (event.get_button() === 1) {
                    if (typeof btn._def.toggle === 'function') {
                        btn._def.toggle();
                        // toggle 后刷新勿扰高亮状态
                        if (btn._def.active) {
                            btn.add_style_pseudo_class('dnd-on');
                        } else {
                            btn.remove_style_pseudo_class('dnd-on');
                        }
                    } else {
                        btn._def.run();
                    }
    ...
    const surface = {
        ...
        onEnter() {
            // 图标从中心点依次缩放淡入；重置位移（避免上次收起的位移残留导致错位）
            ...
            // 录音中：高亮录音按钮（录音现在是第 2 个按钮，index=1）
            if (isRecordingAudio()) {
                buttons[1]?.add_style_pseudo_class('recording');
            } else {
                buttons[1]?.remove_style_pseudo_class('recording');
            }
