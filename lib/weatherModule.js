...
export function createWeatherModule({dock, ext}) {
    ...
    const title = new St.Label({text: '天气', style_class: 'floedock-module-title'});
    ...
    // --- 当前天气 ---
    const nowRow = new St.BoxLayout({style_class: 'floedock-weather-now'});
    const nowIcon = new St.Icon({icon_size: 56, style_class: 'floedock-weather-now-icon'});
    ...
    // --- 3 天预报 ---
    const forecastRow = new St.BoxLayout({
        style_class: 'floedock-weather-forecast',
        x_expand: true,
    });
    content.add_child(forecastRow);
    ...
    function setDetail(key, value) {
        const row = new St.BoxLayout({style_class: 'floedock-weather-detail'});
    ...
    function fillWttr(data) {
        ...
        const forecast = data.weather?.slice(1, 4) ?? [];
        if (!cur)
            return false;
        ...
        for (const day of forecast) {
            const card = new St.BoxLayout({
                style_class: 'floedock-weather-day',
                orientation: Clutter.Orientation.VERTICAL,
                x_expand: true,
                x_align: Clutter.ActorAlign.CENTER,
            });
            ...
            const dayLabel = new St.Label({
                text: `${mm}/${dd}`,
                style_class: 'floedock-weather-day-label',
            });
            card.add_child(dayLabel);
            const icon = new St.Icon({
                icon_name: wttrIcon(Number(day.hourly?.[0]?.weatherCode)),
                icon_size: 24,
            });
            card.add_child(icon);
            const range = new St.Label({
                text: `${day.maxtempC ?? '--'}° ${day.mintempC ?? '--'}°`,
                style_class: 'floedock-weather-day-range',
            });
            card.add_child(range);
            const desc = new St.Label({
                text: day.hourly?.[0]?.weatherDesc?.[0]?.value ?? '',
                style_class: 'floedock-weather-day-desc',
            });
            card.add_child(desc);
            forecastRow.add_child(card);
        }
        return true;
    }
    ...
    function clearDetailGrid() {
        detailGrid.destroy_all_children();
        forecastRow.destroy_all_children();
    }
