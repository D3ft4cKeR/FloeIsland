...
export function createWeatherModule({dock, ext}) {
    ...
    // 无标题栏：root 直接是垂直 BoxLayout，内容区（scroll）占满整个模块
    ...
    // 内容区可滚动：viewport 撑满宽度，内容占满整行（每次打开自动重新加载，无需刷新按钮）
    ...
    // 内容态（不 y_expand：自然高度，配合外层 scroll 可滚动）
    ...
    // 右侧信息列：体感 / 湿度 / 风速 / 紫外线（常驻显示，一眼可见）
    const nowExtra = new St.BoxLayout({
        style_class: 'floedock-weather-now-extra',
        orientation: Clutter.Orientation.VERTICAL,
        x_align: Clutter.ActorAlign.END,
        y_align: Clutter.ActorAlign.CENTER,
    });
    const feelsLabel = new St.Label({
        style_class: 'floedock-weather-now-extra-label',
        x_align: Clutter.ActorAlign.END,
    });
    const humidLabel = new St.Label({
        style_class: 'floedock-weather-now-extra-label',
        x_align: Clutter.ActorAlign.END,
    });
    ...
    const uvLabel = new St.Label({
        style_class: 'floedock-weather-now-extra-label',
        x_align: Clutter.ActorAlign.END,
    });
    nowExtra.add_child(feelsLabel);
    nowExtra.add_child(humidLabel);
    nowExtra.add_child(windLabel);
    nowExtra.add_child(uvLabel);
    nowRow.add_child(nowExtra);
    ...
    function fillWttr(data) {
        ...
        feelsLabel.text = `体感 ${cur.FeelsLikeC ?? '--'}°`;
        humidLabel.text = `湿度 ${cur.humidity ?? '--'}%`;
        windLabel.text = `风 ${cur.windspeedKmph ?? '--'} km/h`;
        uvLabel.text = `紫外线 ${cur.uvIndex ?? '--'}`;
    ...
    function fillOwm(data) {
        ...
        feelsLabel.text = `体感 ${Math.round(cur.main?.feels_like ?? 0)}°`;
        humidLabel.text = `湿度 ${cur.main?.humidity ?? '--'}%`;
        windLabel.text = `风 ${Math.round((cur.wind?.speed ?? 0) * 3.6)} km/h`;
        uvLabel.text = '紫外线 --';
