...
export function createMessagesModule({dock, ext}) {
    ...
    // 消息列表滚动：累积滚轮距离，攒够门槛再用 ease 平滑移动，
    // 避免 smooth 事件一次性大幅跳动（一脚到底）也不卡顿。
    let scrollAcc = 0;
    scroll.connect('scroll-event', (actor, event) => {
        ...
        scrollAcc += Math.sign(dy) * 30;
        if (Math.abs(scrollAcc) >= 48) {
            const target = clamp(adj.value + scrollAcc, adj.lower, max);
            adj.ease(target, {
                duration: 120,
