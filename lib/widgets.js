    ...
    return {
        ...
        setPixbuf(pixbuf) {
            ...
            try {
                ...
                logError(e, '[floeisland] set_pixbuf');
            }
        },
        setFile(file) {
            try {
                ...
                logError(e, '[floeisland] set_file');
            }
        },
        setData(bytes) {
            try {
                ...
                logError(e, '[floeisland] set_data');
            }
        },
        clear() {
            try {
...
export function makeProgressBar({width = 120, height = 6} = {}) {
    ...
    const setValue = v => {
        ...
        fill._last = frac;
        // 用 width 属性（固定尺寸）确保实际生效；CSS 宽度可能被属性覆盖
        fill.width = Math.round(frac * width);
        fill.set_style(`
            background-color: ${fill._accent ?? '#ffffff'};
            border-radius: ${Math.round(height / 2)}px;
            height: ${height}px;
        `);
    };
