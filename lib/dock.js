...
class FloeIslandButton extends PanelMenu.Button {
    ...
    _applyMorphFrame(r) {
        ...
        // 圆角：打开式 胶囊→radius（dir=1），收起式 radius→胶囊999（dir=-1）。
        // 前 25%（r<0.25）快速过渡到目标圆角，避免"宽大圆角扁胶囊"或收起先闪方/岛状
        const rq = Math.min(1, r * 4);
        const rv = this._morphDir === 1
            ? Math.round(999 + (radius - 999) * rq)
            : Math.round(radius + (999 - radius) * rq);
