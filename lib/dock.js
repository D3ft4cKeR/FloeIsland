...
class FloeDockButton extends PanelMenu.Button {
     ...
     * 打开流程（串行，每段完整跑完才进下一段，无跳变、不闪现）：
     *   1. 时钟淡化
     *   2. 胶囊以"初始胶囊中心"为固定横轴，左右横向拉长到目标宽（露出搜索栏）
     *   3. 胶囊向下拉出到目标高（露出模块）
     *   4. 内容淡入
     */
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        this._animCancel();
        // 强制确定起点：先显式置为 Dock 态胶囊尺寸，再读取，避免陈旧/未布局尺寸
        ...
        onStage?.('pull-start');
        // 打开 waypoint 序列：横拉(宽不变高) → 下拉(高到目标)
        const stages = [
            {w: toW, h: fromH, dur: 360, mode: 'cubic'}, // 横向拉长
            {w: toW, h: toH, dur: 400, mode: 'cubic'},   // 向下拉出
        ];
        this._drivePanelMorph(stages, () => {
    ...
    _panelCapsuleBack({onDone}) {
        ...
        // 收起 waypoint：先 向上缩回高度（宽度保持面板宽）→ 再 向中间缩回宽度
        const stages = [
            {w: null, h: capsuleH, dur: 340, mode: 'in'}, // 仅高度缩（宽不变）
            {w: capsuleW, h: capsuleH, dur: 300, mode: 'in'}, // 仅宽度缩
        ];
        this._drivePanelMorph(stages, () => {
    ...
    /**
     * 面板形变通用串行驱动：按 waypoint 数组依次插入，每段从"当前胶囊几何"插值
     * 到目标（w/h 为 null 表示该轴保持当前不变）。横轴始终锚定 _panelCenterX。
     */
    _drivePanelMorph(stages, onComplete) {
        let i = 0;
        const step = () => {
            if (i >= stages.length) {
                onComplete?.();
            ...
            i++;
            const toW = s.w == null
                ? Math.max(DOCK_MIN_WIDTH, this._capsule.get_width())
                : s.w;
            const toH = s.h == null
                ? Math.max(DOCK_MIN_HEIGHT, this._capsule.get_height())
                : s.h;
            this._runPanelPhase(toW, toH, s.dur, s.mode, step);
        };
        step();
    }

    /** 面板形变单段插值：从当前胶囊尺寸插值到 (toW,toH)，横轴中心锚定。 */
    _runPanelPhase(toW, toH, duration, mode, onComplete) {
        ...
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            ...
            this._positionNotifDots(x, y0, w, h);
            if (t >= 1) {
                this._anim = null;
                onComplete?.();
                return GLib.SOURCE_REMOVE;
