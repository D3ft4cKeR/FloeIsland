...
class FloeDockButton extends PanelMenu.Button {
    ...
    _transitionTo(name, params) {
        ...
        // 只有通知态用胶囊形变；面板用浮层两段式（先横拉再下拉，更稳不卡）
        const inCapsule = name === State.NOTIFICATION;
        ...
        if (name === State.DOCK) {
            ...
            if (this._collapseFromNotif) {
                // 通知态内容淡出、时钟淡入，随反向形变同时进行
            ...
            // 通知态收起：胶囊保持可见（反向形变归位）；
            // 面板/工具栏等浮层态收起：胶囊由 _collapseToDock 淡入（_showCapsule）
            if (old === State.NOTIFICATION) {
                this._capsule.opacity = 255;
                this._capsule.scale_x = 1;
                this._capsule.scale_y = 1;
        ...
        // 其他展开态（面板/工具栏/字幕）：浮层从胶囊展开。
        // 面板用两段式"先横拉再下拉"（_expandTo），一步到位全宽、向下拉出，
        // 避免胶囊形变带来的重布局卡顿与工具栏误触发。
        if (old === State.NOTIFICATION)
