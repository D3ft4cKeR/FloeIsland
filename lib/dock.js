...
export const FloeDockButton = GObject.registerClass({
    ...
    _transitionTo(name, params) {
        ...
        const onExpandDone = () => {
            this._transitioning = false;
            this._floatLayer.clip_to_allocation = false; // 展开完成：取消裁剪
        ...
        // 从 Dock 展开（两阶段，不依赖 pivot/notify）：
        //   阶段一：floatWidth 从胶囊宽 → 全宽；动画期间用定时轮询强制把
        //          浮层位置重新居中（位置 = 按钮中心 - 宽/2），必然左右对称。
        //          （ease_property 动画自定义 GObject 属性时 notify 不可靠，
        //           所以不能依赖 notify::float-width 触发重定位。）
        //   阶段二：floatHeight 从胶囊高 → 全高（向下打开）。
        this._floatLayer.clip_to_allocation = true;
        this.floatWidth = this.dockWidth ?? DOCK_MIN_WIDTH;
        this.floatHeight = DOCK_MIN_HEIGHT;
        this._syncFloatGeometry();
        this._startFloatSync();
        surf.onExpandStage?.('pull-start');
        easeProperty(this, 'float-width', size.width, {
            ...anim,
            onComplete: () => {
                surf.onExpandStage?.('pull-done');
                easeProperty(this, 'float-height', size.height, {
                    ...anim,
    ...
    /** 动画期间每 16ms 强制同步浮层几何（位置居中），保证左右对称。 */
    _startFloatSync() {
        this._stopFloatSync();
        this._floatSyncId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            this._syncFloatGeometry();
            return GLib.SOURCE_CONTINUE;
        });
        GLib.Source.set_name_by_id(this._floatSyncId, '[floedock] float sync');
    }

    _stopFloatSync() {
        if (this._floatSyncId) {
            GLib.source_remove(this._floatSyncId);
            this._floatSyncId = 0;
        }
    }
    ...
    _applyTheme() {
        ...
        const glow = clamp(16 + blur / 4, 16, 50);
        ...
        // 胶囊内联样式（深色玻璃：暗底渐变 + 真背景模糊 + 柔和阴影）
        this._capsule.set_style(`
            background-gradient-direction: vertical;
            background-gradient-start: rgba(32,39,54,${this._theme.topAlpha});
            background-gradient-end: rgba(9,12,19,${this._theme.baseAlpha});
