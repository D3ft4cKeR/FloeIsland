...
class FloeDockButton extends PanelMenu.Button {
    ...
    /**
     * 通知展开动画：岛体整体下移突破顶栏，同时变宽+变高成为通知卡片。
     * 整个过程：下移 + 扩宽 + 拉高 同时进行。
     */
    _expandNotification(toW, toH, {duration, mode, onStage, onDone}) {
        log(`[floedock] _expandNotification: toW=${toW} toH=${toH} duration=${duration}`);
        this._animCancel();
        const fromW = Math.max(DOCK_MIN_WIDTH, this._floatLayer.get_width());
        const fromH = this._capsule.get_height() || DOCK_MIN_HEIGHT;
        this._floatLayer.clip_to_allocation = true;
        this._notifPullY = 0; // 下移偏移量（0 → PULL_DOWN）
        onStage?.('pull-start');
        // 下移 + 扩宽 + 拉高 同时进行
        this._animateNotifFull(fromW, fromH, toW, toH, 0, 36, duration, mode, () => {
            onStage?.('pull-done');
            onDone?.();
        });
    }
    ...
    /**
     * 通知态完整动画：宽度 + 高度 + y偏移（下移/上移）同时变化。
     * pullFrom/pullTo: y偏移量的起止值（展开 0→36，收起 36→0）。
     */
    _animateNotifFull(fromW, fromH, toW, toH, pullFrom, pullTo, duration, mode, onDone) {
        log(`[floedock] _animateNotifFull: from=${fromW}x${fromH} pull=${pullFrom}→${pullTo}`);
        const ease = easeFor(mode);
        const t0 = Date.now();
        let logged = false;
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            ...
            const w = Math.round(fromW + (toW - fromW) * e);
            const h = Math.round(fromH + (toH - fromH) * e);
            const pullY = Math.round(pullFrom + (pullTo - pullFrom) * e);
            this._notifPullY = pullY;
            this._floatLayer.set_size(w, h);
            this._positionFloatNotif(w, h);
            if (!logged) {
                logged = true;
                log(`[floedock] anim1: w=${w} h=${h} pullY=${pullY} visible=${this._floatLayer.visible} parent=${this._floatLayer.get_parent()?.constructor?.name}`);
            }
            if (t >= 1) {
                log(`[floedock] anim done: final w=${w} h=${h} pullY=${pullY}`);
                this._anim = null;
                onDone?.();
                return GLib.SOURCE_REMOVE;
            }
    ...
    _collapseToDock() {
        ...
        if (fromNotif) {
            ...
            const pullFrom = this._notifPullY ?? 36;
            this._animateNotifFull(w0, h0, this.dockWidth, capsuleH, pullFrom, 0, 300, 'in', () => {
                this._notifPullY = 0;
    ...
    _transitionTo(name, params) {
        ...
        if (fromDock) {
            ...
            if (name === State.NOTIFICATION) {
                ...
                log(`[floedock] calling _expandNotification size=${size.width}x${size.height}`);
