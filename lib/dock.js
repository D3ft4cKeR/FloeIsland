...
class FloeDockButton extends PanelMenu.Button {
     ...
     * 严格时序（每段完整跑完才进下一段，不闪现）：
     *   阶段 A（360ms）：时钟淡出 与 岛横向拉长 严格同时进行（共用一个逐帧插值）
     *   阶段 B（420ms）：岛 纵向拉开 到目标高（露出模块）
     *   阶段 C（260ms）：内容淡入
     * 全程以"初始胶囊中心"为固定横轴，水平永不漂移。
     */
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        this._animCancel();
        // 起点 = Dock 态胶囊尺寸（用 preferred 真源，不读可能旧/未布局的 get_*）
        const fromW = Math.max(DOCK_MIN_WIDTH, this._notifFromW || this.dockWidth);
        const [, prefH] = this._capsule.get_preferred_height(-1);
        const fromH = Math.max(DOCK_MIN_HEIGHT, prefH || DOCK_MIN_HEIGHT);
        this._capsule.set_size(fromW, fromH);
        this._capsule.clip_to_allocation = true;
        // 固定横轴中心（提升到 uiGroup 后的绝对中心）
        this._panelCenterX = this._capsule.get_x() + fromW / 2;
        this._panelAnchorY = this._capsule.get_y();
        ...
        onStage?.('pull-start');
        // 阶段 A：横拉（高度恒 fromH）+ 时钟淡出同拍
        this._fuseClockAndStretch(fromW, toW, fromH, 360, radius, () => {
            // 阶段 B：纵向拉开（宽度保持 toW）→ 阶段 C 内容淡入
            this._stretchHeight(toW, fromH, toH, 420, radius, () => {
    ...
    _panelCapsuleBack({onDone}) {
        ...
        this._stretchHeight(panelW, panelH, capsuleH, 340, Math.max(12, this._theme?.radius ?? 14), () => {
     ...
     */
    _fuseClockAndStretch(fromW, toW, fixedH, duration, radius, onComplete) {
        ...
        GLib.Source.set_name_by_id(this._anim, '[floedock] panel stretch');
    }

    /**
     * 阶段 B：岛纵向拉开（宽度保持 fixedW），单一逐帧插值（圆角离散更新）。
     */
    _stretchHeight(fixedW, fromH, toH, duration, radius, onComplete) {
        const ease = easeFor('cubic');
        const t0 = Date.now();
        ...
        GLib.Source.set_name_by_id(this._anim, '[floedock] panel stretchH');
    }
    ...
    _shrinkBackWidth(fromW, toW, fixedH, duration, onComplete) {
        ...
        GLib.Source.set_name_by_id(this._anim, '[floedock] panel shrinkW');
    }
