...
class FloeDockButton extends PanelMenu.Button {
    ...
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        this._animCancel();
        const dockW = Math.max(DOCK_MIN_WIDTH, this._notifFromW || this.dockWidth);
        const dockH = Math.max(DOCK_MIN_HEIGHT, this._capsule.get_height() || DOCK_MIN_HEIGHT);
        this._capsule.set_size(dockW, dockH);
    ...
    _fuseClockAndStretch(fromW, toW, fixedH, duration, radius, onComplete) {
        ...
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            ...
            const e = ease(t);
            const w = Math.round(fromW + (toW - fromW) * e);
            const rv = Math.round(999 + (radius - 999) * e);
            ...
            this._positionNotifDots(x, y0, w, fixedH);
    ...
    _stretchHeight(fixedW, fromH, toH, duration, radius, onComplete) {
        ...
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            ...
            this._capsule.set_style(this._notifStyle(rv));
            this._positionNotifDots(x, y0, fixedW, h);
            if (t >= 1) {
                this._anim = null;
                onComplete?.();
                return GLib.SOURCE_REMOVE;
