...
class FloeDockButton extends PanelMenu.Button {
    ...
    /** 通知态结束：胶囊回岛、恢复初始岛态（时钟/样式/圆角），隐藏通知内容与指示点。 */
    ...
    _collapseToDock() {
        ...
        // 面板/工具栏收起：胶囊淡入（与浮层缩回同时进行），避免"突然出现"闪烁
        this._showCapsule();
    ...
    _transitionTo(name, params) {
        ...
        if (name === State.DOCK) {
            ...
            // 通知态收起：胶囊保持可见（反向形变归位）；面板/工具栏收起：
            // 胶囊由 _collapseToDock 淡入（_showCapsule），不在这里瞬时弹出
            if (old === State.NOTIFICATION) {
                this._capsule.opacity = 255;
                this._capsule.scale_x = 1;
                this._capsule.scale_y = 1;
