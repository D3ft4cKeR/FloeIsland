...
class FloeIslandButton extends PanelMenu.Button {
    ...
    _applyMorphFrame(r) {
        ...
        // 圆角：打开式 胶囊→radius（dir=1，前25%快）；收起式 圆角与"尺寸完成度 e"同步回999，
        // 让尺寸缩到胶囊时圆角同时闭合 —— 全程是"圆角适中的方块在缩小"，不会出现
        // "圆角已999但尺寸还大"的中途大胶囊，一步到位成岛胶囊。
        let rv;
        if (this._morphDir === 1) {
            const rq = Math.min(1, r * 4);
            rv = Math.round(999 + (radius - 999) * rq);
        } else {
            rv = Math.round(radius + (999 - radius) * e);
        }
    ...
    _morphCapsuleExpand(toW, toH, {radius = 12, pullY = NOTIF_PULL, duration = 400, onStage, onDone}) {
        ...
        this._capsule.set_size(fromW, fromH);
        this._capsule.set_position(Math.round(curX), Math.round(curY));
        // 固定横轴中心（dock 中心）：优先用 _capsuleToTopCentered 缓存的岛 dock 中心
        this._dockCenterX = this._dockCenterX ?? (curX + fromW / 2);
        this._panelCenterX = this._dockCenterX;
        this._panelAnchorY = curY;
        this._notifFromW = fromW;
        this._notifFromH = fromH;
    ...
    _morphCapsuleBack({duration = 300, onDone}) {
        ...
        // 保证收起后精确还原为「当前 dock 胶囊尺寸」（与 _capsuleBackToIsland 一致，避免突变）
        const toW = Math.max(DOCK_MIN_WIDTH, this._dockWidth || DOCK_MIN_WIDTH);
        const toH = DOCK_MIN_HEIGHT;
    ...
    _capsuleToTopCentered() {
        const fromW = Math.max(DOCK_MIN_WIDTH, this._capsule.get_width());
        const fromH = this._capsule.get_height() || DOCK_MIN_HEIGHT;
        // 锚点 = 岛的 dock 位置（island 在面板中心 statusArea，即岛的横轴中心），
        // 而非主屏几何中心——否则通知展开/收起会以偏位为轴，收起时偏离岛、归位跳变。
        const [ix, iy] = this._island.get_transformed_position();
        const iw = this.dockWidth || fromW;   // 岛宽 = dock 胶囊宽（IslandLayout 中心布局）
        const panelH = Main.panel ? Main.panel.get_height() : 0;
        const cx = ix + iw / 2;
        this._dockCenterX = Math.round(cx);           // 缓存岛 dock 中心，供收起归位对齐
        this._panelCenterX = this._dockCenterX;        // 通知/面板形变统一用这个中心锚
        this._panelAnchorY = Math.round(panelH - fromH / 2);
        const x = Math.round(cx - fromW / 2);
        const y = this._panelAnchorY;
        this._reparent(this._capsule, Main.uiGroup);
        Main.uiGroup.set_child_above_sibling(this._capsule, null);
        this._capsule.set_position(x, y);
    }
