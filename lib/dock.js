...
class FloeDockButton extends PanelMenu.Button {
     ...
     * 面板态：radius 主题圆角、pullY 0（从顶栏向下展开成面板）。
    ...
    _collapseToDock() {
        ...
        if (fromNotif || this._capsule.get_parent() === Main.uiGroup) {
            // 通知/面板态收起：缩回宽度+高度 同时 上移归位（一个阶段，同演示）
    ...
    _transitionTo(name, params) {
        ...
        if (name === State.DOCK) {
            ...
            const fromCapsule = old === State.NOTIFICATION || old === State.PANEL;
            if (fromCapsule) {
                // 胶囊形变态（通知/面板）内容淡出、时钟淡入，随反向形变同时进行
                const inst = this._instances.get(old);
                inst?.widget?.ease({
        ...
        if (name === State.NOTIFICATION || name === State.PANEL) {
            // 胶囊形变态（通知/面板）：不用浮层——胶囊本身下移+变宽+变高成为界面
            if (this._capsuleOverride) {
            ...
            if (name === State.NOTIFICATION) {
                // 通知内容随形变淡入；面板内容由 openPanel 自行淡入
            ...
            // 一步到位：胶囊直接形变到目标尺寸（不再先"迷你岛"再纠正）
            const isPanel = name === State.PANEL;
            this._morphCapsuleExpand(size.width, size.height, {
                radius: isPanel ? Math.max(12, this._theme?.radius ?? 14) : 12,
                pullY: isPanel ? 0 : NOTIF_PULL,
