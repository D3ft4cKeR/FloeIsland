...
class FloeDockButton extends PanelMenu.Button {
    ...
    /**
     * 通知展开动画（胶囊形变，对齐 preview/notif-demo.html）：
     * 岛体整体下移 NOTIF_PULL px 突破顶栏，同时胶囊本身
     * 变宽 + 变高 + 圆角 999→12，通知内容在胶囊内部随形变淡入。
     */
    _morphCapsuleToNotif(toW, toH, {duration, mode, onStage, onDone}) {
        this._animCancel();
        ...
        this._notifPullY = 0; // 下移偏移量（0 → NOTIF_PULL）
        const ease = easeFor(mode);
        const t0 = Date.now();
        onStage?.('pull-start');
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
        ...
        GLib.Source.set_name_by_id(this._anim, '[floedock] notif morph');
    }

    /**
     * 通知收起动画：反向形变——上移归位 + 缩回宽度 + 缩短高度 同时进行，
     * 圆角 12→999（过半后恢复胶囊圆角，同演示）。
     */
    _morphCapsuleBack({duration, mode, onDone}) {
        this._animCancel();
        const fromW = Math.max(DOCK_MIN_WIDTH, this._capsule.get_width());
        ...
        GLib.Source.set_name_by_id(this._anim, '[floedock] notif collapse');
    }
    ...
    /**
     * 通知态浮层定位：岛体从胶囊位置整体下移 pullY 像素，
     * 同时水平居中于胶囊。pullY=0 时贴紧顶栏，pullY>0 时突破下边界。
     */
    _positionFloatNotif(fw, fh) {
        ...
        const pullY = this._notifPullY ?? 0;
        const top = Math.round(cy + pullY); // 从胶囊顶开始，向下偏移突破顶栏
    ...
    _collapseToDock() {
        ...
        if (fromNotif) {
            // 通知态收起：缩回宽度+高度 同时 上移归位（一个阶段，同演示）
    ...
    _transitionTo(name, params) {
        ...
        const surf = name === State.DOCK
            ? null
            : this._getSurface(name, name === State.NOTIFICATION ? this._capsule : this._floatLayer);
        if (name !== State.DOCK && !surf)
            return;

        // 离开旧状态（必须调 onLeave，否则 surface 的捕获等不清理；
        // 通知态内容不立即 hide——由反向形变动画淡出，dock 里统一处理）
        if (old !== State.DOCK) {
        ...
        if (name === State.DOCK) {
            ...
            if (old === State.NOTIFICATION) {
                // 通知内容/指示点淡出、时钟淡入，随反向形变同时进行
                const ni = this._instances.get(State.NOTIFICATION);
                ni?.widget?.ease({
        ...
        if (name === State.NOTIFICATION) {
            ...
            this._morphCapsuleToNotif(size.width, size.height, {
        ...
        // 其他展开态：浮层从胶囊展开
        if (old === State.NOTIFICATION)
            this._capsuleBackToIsland(); // 防御：通知态直接切走时复位胶囊
