...
class FloeIslandButton extends PanelMenu.Button {
    ...
    /**
     * 启动一次胶囊形变：ease_property 驱动 this.morph-r（恒 0→1，代表 from→to 尺寸）。
     * frame={fromW,fromH,toW,toH,radius}。direction=1 打开（时钟淡出），0 收起（时钟淡入）。
     */
    _runCapsuleMorph(frame, {direction = 1, duration = 400, mode = 'cubic', onDone} = {}) {
        ...
        this._morphTask = this.ease_property('morph-r', 1, {
            duration,
            mode: this._modeToAnimation(mode),
            onComplete: () => {
    ...
    /** 根据 morph-r（0→1）刷新胶囊几何。
     * 面板(stage 默认)：横向占 0→0.5、纵向占 0.5→1（先宽后高）。
     * 通知(simul)：宽高用同一 e 同步增长，y 同时下移 pullY，并跟随堆叠点。
     * frame: {fromW,fromH,toW,toH,radius, simul?, pullY?}；_morphDir 决定时钟淡出/入。 */
    _applyMorphFrame(r) {
        const f = this._morphFrame;
        if (!f)
            return;
        const {fromW, fromH, toW, toH, radius, simul, pullY = 0} = f;
        ...
        if (simul) {
            ...
            const segW = Math.min(1, Math.max(0, r * 2));
            const segH = Math.min(1, Math.max(0, (r - 0.5) * 2));
            ew = easeFor('cubic')(segW);
            eh = easeFor('cubic')(segH);
            this._notifPullY = 0;
        }
        const w = Math.round(fromW + (toW - fromW) * ew);
        const h = Math.round(fromH + (toH - fromH) * eh);
        const yPull = simul ? Math.round(pullY * e) : 0;
        const cx = this._panelCenterX ?? (fromW / 2);
        const yAnchor = this._panelAnchorY ?? this._capsule.get_y();
        ...
        // 圆角：前 25%（r<0.25）快速降到目标
        const rq = Math.min(1, r * 4);
        const rv = Math.round(999 + (radius - 999) * rq);
        this._capsule.set_style(this._notifStyle(rv));
        // 时钟：打开（dir=1）淡出 255→0；收起（dir=-1）淡入 0→255
    ...
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        ...
        onStage?.('pull-start');
        // 合成器驱动的胶囊形变：r 0→1，内部"先横拉(前半)→再纵拉(后半)"，内容不动、不用 scale
    ...
    _morphCapsuleExpand(toW, toH, {radius = 12, pullY = NOTIF_PULL, duration = 400, onStage, onDone}) {
        ...
        onStage?.('pull-start');
        // 合成器驱动：simul 模式下 宽/高/下移 用同一 e 同步增长，内容不动、不用 scale
        this._runCapsuleMorph(
            {fromW, fromH, toW, toH, radius, simul: true, pullY},
            {direction: 1, duration, mode: 'cubic', onDone: () => {
                onStage?.('pull-done');
                onDone?.();
            }});
    }
    ...
    _morphCapsuleBack({duration = 300, onDone}) {
        ...
        this._notifPullY = 0;
        // 合成器驱动：simul 同步缩回 + 时钟淡入，结束后归位
        this._runCapsuleMorph(
            {fromW, fromH, toW, toH, radius, simul: true, pullY: 0},
            {direction: 0, duration, mode: 'in', onDone});
    }
