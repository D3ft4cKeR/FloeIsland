...
class FloeDockButton extends PanelMenu.Button {
    ...
    /**
     * 通知态专用展开：苹果灵动岛风格。
     * 浮层从胶囊位置"向下突破"顶栏 —— 上边框始终贴住顶栏下边，
     * 高度从 0 增长到目标高度，同时宽度从胶囊宽左右拉伸到通知宽。
     * 定位由 _positionFloatNotif 处理（y 固定在顶栏底部）。
     */
    _expandNotification(toW, toH, {duration, mode, onStage, onDone}) {
        ...
        this._floatLayer.clip_to_allocation = true;
        onStage?.('pull-start');
        // 阶段一：向下突破（高度 0→目标），宽度保持胶囊宽
        this._animateSizePhaseNotif(fromW, 0, fromW, toH,
    ...
    /**
     * 通知态浮层定位：上边框贴住顶栏下边（向下突破顶栏），
     * 水平居中于胶囊中心。
     */
    _positionFloatNotif(fw, fh) {
        if (this._floatLayer.get_parent() !== Main.uiGroup)
            return;
        const [cx] = this._capsule.get_transformed_position();
        const cw = this._capsule.get_width();
        const w = fw ?? this._floatLayer.get_width();
        const top = Main.panel.get_height() - 1; // 顶栏底部（贴下边）
    ...
    _collapseToDock() {
        ...
        if (fromNotif) {
            // 通知态收起：原路返回 —— 先左右缩小到胶囊宽，再上升缩回（顶栏底锚点）
            ...
            this._animateSizePhaseNotif(w0, h0, this.dockWidth, h0, 160, 'in', () => {
                this._animateSizePhaseNotif(this.dockWidth, h0, this.dockWidth, 0, 140, 'in', () => {
