...
class FloeIslandButton extends PanelMenu.Button {
    ...
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        ...
        // 胶囊 dock 宽 = 时钟宽(_dockWidth，正常 <300)；若被污染成面板宽则回退常量
        const rawDockW = this._dockWidth;
        const fromW = (rawDockW > 0 && rawDockW < 300)
            ? Math.round(rawDockW)
            : DOCK_MIN_WIDTH;
        const fromH = DOCK_MIN_HEIGHT; // 胶囊 dock 高：固定常量
        const [curX, curY] = this._capsule.get_transformed_position();
        // 先把胶囊强制放到 dock 小胶囊尺寸再定位，确保第一帧就是"未拉伸的小胶囊"
        ...
        // 固定横轴中心 = 当前胶囊中心（dock 中心），后续两段都以此为中心向两侧/向下
        ...
        onStage?.('pull-start');
        // 段A：横向拉开（高度恒 fromH = dock 高）→ 段B：纵向拉开
        this._animatePanelWide(fromW, toW, fromH, 360, radius, () => {
            this._animatePanelTall(toW, fromH, toH, 420, radius, () => {
     ...
     *   1. 内容淡出
     *   2. 向上缩回高度（中心横轴不变）→ 向中间缩回宽度 → 时钟淡入/归位
     */
    _panelCapsuleBack({onDone}) {
        ...
        const capsuleW = this._notifFromW || this.dockWidth;
        const capsuleH = this._notifFromH || DOCK_MIN_HEIGHT;
        // 反向：步骤1 纵向缩回高度（宽度保持面板宽）→ 步骤2 横向缩回宽度+时钟淡入
        const panelW = this._capsule.get_width() || capsuleW;
        const panelH = this._capsule.get_height() || DOCK_MIN_HEIGHT;
        this._animatePanelTall(panelW, panelH, capsuleH, 340, Math.max(12, this._theme?.radius ?? 14), () => {
            this._shrinkBackWidth(panelW, capsuleW, capsuleH, 300, () => {
                this._panelIsOpen = false;
                this._notifPullY = 0;
                this._capsuleBackToIsland();
                onDone?.();
    ...
    /**
     * 阶段 A：岛横向拉长 + 时钟淡出 严格同拍（单一逐帧插值，共用同一 e）。
     * 性能：圆角只在变化≥1px 时更新，避免每帧全量 set_style 造成卡顿。
     */
    _animatePanelWide(fromW, toW, fixedH, duration, radius, onComplete) {
        const ease = easeFor('cubic');
        const t0 = Date.now();
        const cx = this._panelCenterX ?? (fromW / 2);
        const y0 = this._panelAnchorY ?? this._capsule.get_y();
        let lastR = -1;
        this._anim = GLib.timeout_add(GLib.PRIORITY_HIGH, 16, () => {
            const t = Math.min(1, (Date.now() - t0) / Math.max(1, duration));
            const e = ease(t);
            const w = Math.round(fromW + (toW - fromW) * e);
            const x = Math.round(cx - w / 2 + this._floatOffsetX);
            this._capsule.set_position(x, y0);
            this._capsule.set_size(w, fixedH);
            // 圆角在前 25% 快速降到目标，避免"宽大圆角扁胶囊"的椭圆观感
            const q = Math.min(1, e * 4);
            const r = Math.round(999 + (radius - 999) * q);
            if (r !== lastR) {
                lastR = r;
                this._capsule.set_style(this._notifStyle(r));
            }
            // 时钟随同一 e 淡出（255 → 0），与横拉完全同步
            this._clockLabel.opacity = Math.round(255 * (1 - e));
            if (t >= 1) {
                this._anim = null;
                this._clockLabel.opacity = 0;
                onComplete?.();
                return GLib.SOURCE_REMOVE;
            }
            return GLib.SOURCE_CONTINUE;
        });
        GLib.Source.set_name_by_id(this._anim, '[floeisland] panel stretch');
    }

    /**
     * 阶段 B：岛纵向拉开（宽度保持 fixedW），单一逐帧插值（圆角离散更新）。
     */
    _animatePanelTall(fixedW, fromH, toH, duration, radius, onComplete) {
        const ease = easeFor('cubic');
        const t0 = Date.now();
        const cx = this._panelCenterX ?? (fixedW / 2);
        const y0 = this._panelAnchorY ?? this._capsule.get_y();
        let lastR = -1;
        this._anim = GLib.timeout_add(GLib.PRIORITY_HIGH, 16, () => {
            const t = Math.min(1, (Date.now() - t0) / Math.max(1, duration));
            const e = ease(t);
            const h = Math.round(fromH + (toH - fromH) * e);
            const x = Math.round(cx - fixedW / 2 + this._floatOffsetX);
            this._capsule.set_position(x, y0);
            this._capsule.set_size(fixedW, h);
            const r = radius; // 纵拉阶段圆角固定为目标值
            if (r !== lastR) {
                lastR = r;
                this._capsule.set_style(this._notifStyle(r));
            }
        ...
        GLib.Source.set_name_by_id(this._anim, '[floeisland] panel stretchH');
    }

    /**
     * 收起：横向缩回宽度 + 时钟淡入 同拍（反向阶段 A 的同 e 驱动）。
     */
    _shrinkBackWidth(fromW, toW, fixedH, duration, onComplete) {
        const ease = easeFor('in');
        const t0 = Date.now();
        ...
        this._anim = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
            ...
            this._capsule.set_position(x, y0);
            this._capsule.set_size(w, fixedH);
            // 时钟随同一 e 淡入（0 → 255），与宽度缩回同拍
            this._clockLabel.opacity = Math.round(255 * e);
            if (t >= 1) {
                this._anim = null;
                this._clockLabel.opacity = 255;
                onComplete?.();
        ...
        GLib.Source.set_name_by_id(this._anim, '[floeisland] panel shrinkW');
    }
    ...
    _morphCapsuleExpand(toW, toH, {radius = 12, pullY = NOTIF_PULL, mode, duration, onStage, onDone}) {
        ...
        GLib.Source.set_name_by_id(this._anim, '[floeisland] capsule morph');
    }
    ...
    _morphCapsuleBack({mode, duration, onDone}) {
        ...
        GLib.Source.set_name_by_id(this._anim, '[floeisland] capsule collapse');
    }
