...
// ---------------------------------------------------------------------------
// FloeIslandLayout：只负责胶囊（按钮宽度 = 胶囊宽度）与迷你小岛（胶囊左侧）。
// 浮层由本文件顶层管理。
// 类名带 Floe 前缀：避免与旧版扩展 / 其它扩展 / Shell 的同名 GType 冲突（否则 GObject 二次注册崩）。
// ---------------------------------------------------------------------------
const MINI_GAP = 8; // 迷你小岛与胶囊的间距

const FloeIslandLayout = GObject.registerClass(
class FloeIslandLayout extends Clutter.LayoutManager {
...
class FloeIslandButton extends PanelMenu.Button {
    _init(ext) {
        ...
        // --- 布局 ---
        this._island = new St.Widget({layout_manager: new FloeIslandLayout()});
     ...
     * 严格两段时序，但用 Clutter 合成器动画器（ease）驱动 scale 变换，
     * 而非逐帧 set_size —— 动画期间零 reflow、零 set_style，GPU 丝滑。
     *   段A：横向拉开（scale_x dock→面板宽/胶囊宽，内容随胶囊横向拉伸）
     *   段B：纵向拉开（scale_y dock→面板高/胶囊高）
     *   完成后 snap 到精确尺寸 + 目标圆角，内容重新布局清晰。
     * 收起为精确反向。
     */
    _panelCapsuleExpand(toW, toH, {radius = 14, onStage, onDone}) {
        this._animCancel();
        // 起点 = Dock 态胶囊尺寸（常量，绝不用残留/未布局值）
        const rawDockW = this._dockWidth;
        const fromW = (rawDockW > 0 && rawDockW < 300) ? Math.round(rawDockW) : DOCK_MIN_WIDTH;
        const fromH = DOCK_MIN_HEIGHT;
        ...
        const sxMax = Math.max(1, toW / fromW);
        const syMax = Math.max(1, toH / fromH);
        onStage?.('pull-start');
        // 段A：横向拉开（scale_x，含时钟淡出同拍）
        ...
        this._capsule.ease({
            ...
            onComplete: () => {
                ...
                this._capsule.ease({
                    ...
                    mode: Clutter.AnimationMode.EASE_OUT_CUBIC,
                    onComplete: () => {
                        // 段C：snap 到精确尺寸 + 目标圆角 + 内容清晰重排
                        this._capsule.set_scale(1, 1);
                        this._capsule.set_size(toW, toH);
                        this._capsule.set_style(this._notifStyle(radius));
                        this._capsule.set_position(
                            Math.round(this._panelCenterX - toW / 2 + this._floatOffsetX),
                            Math.round(this._panelAnchorY));
    ...
    /**
     * 面板态胶囊形变收起（精确反向）：内容淡出 → scale_y 纵向缩回 → scale_x 横向缩回
     * → snap 回 Dock 尺寸 + 时钟淡入。
     */
    _panelCapsuleBack({onDone}) {
        this._animCancel();
        this._instances.get(State.PANEL)?.widget?.ease({
            opacity: 0, duration: 140, mode: Clutter.AnimationMode.EASE_IN_CUBIC,
        });
        ...
        const panelH = this._capsule.get_height() || capsuleH;
        // 从面板尺寸缩回胶囊：需要的 scale 因子（当前胶囊已是面板尺寸，scale=1）
        // 这里改为 scale 收缩动画后 snap 回 dock
        this._capsule.ease({
            scale_y: Math.max(0.0001, capsuleH / panelH),
            duration: 320,
            mode: Clutter.AnimationMode.EASE_IN_CUBIC,
            onComplete: () => {
                this._capsule.ease({
                    scale_x: Math.max(0.0001, capsuleW / panelW),
                    duration: 280,
                    mode: Clutter.AnimationMode.EASE_IN_CUBIC,
                    onComplete: () => {
                        this._capsule.set_scale(1, 1);
                        this._capsule.set_size(capsuleW, capsuleH);
                        this._capsule.set_style(this._notifStyle(999));
