...
export const FloeDockButton = GObject.registerClass({
    ...
    _transitionTo(name, params) {
        ...
        if (old !== State.DOCK && name !== State.DOCK) {
        ...
        easeProperty(this, 'float-height', size.height, {
            ...anim,
            duration: Math.round(anim.duration * 1.1),
        });
    }
    ...
    _setupInput() {
        // GNOME 50 的 Clutter.ClickGesture 使用 recognize 信号（无 released）
        this._clickGesture = new Clutter.ClickGesture();
        this._clickGesture.connect('recognize', () => {
            if (this._clickGesture.get_button() !== 1)
                return;
    ...
    _applyTheme() {
        ...
        const border = clamp(36 + blur / 2, 36, 90);
        ...
        this._theme = {
            ...
            fontSize,
            fontFamily,
            blurRadius: blur > 0 ? Math.max(4, Math.round(blur * 0.4)) : 0,
        ...
        // 胶囊内联样式（半透明渐变打底，配合 Shell.BlurEffect 真背景模糊）
        this._capsule.set_style(`
            background-gradient-direction: vertical;
            background-gradient-start: rgba(255,255,255,${this._theme.topAlpha});
            background-gradient-end: rgba(255,255,255,${this._theme.baseAlpha});
    ...
    /** 给 actor 应用/更新 Shell.BlurEffect 真背景模糊（BACKGROUND 模式）。 */
    _applyBlur(actor) {
