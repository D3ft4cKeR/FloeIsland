...
export const FloeDockButton = GObject.registerClass({
    ...
    _init(ext) {
        ...
        this._clockLabel.connect('notify::text', () => this._updateDockWidth());
    ...
    _setupInput() {
        ...
        this._clickGesture.connect('released', (action, actor, nPress) => {
            if (nPress !== 1 || action.get_button() !== 1)
                return;
    ...
    _applyTheme() {
        ...
        // 胶囊内联样式（St 无 backdrop-filter，用多层半透明渐变模拟浮冰玻璃）
