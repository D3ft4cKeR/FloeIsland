// 秒表模块：左右结构 —— 左大时间（完整显示）+ 秒表图标 + 计次列表（最多 3 条，
...
export function createStopwatchModule({dock, ext}) {
    ...
    const icon = new St.Icon({
        icon_name: 'chronometer-symbolic',
        icon_size: 22,
        style_class: 'floedock-timer-icon',
        y_align: Clutter.ActorAlign.CENTER,
    });
    timeRow.add_child(icon);
    ...
    timeRow.add_child(display);
    leftCol.add_child(timeRow);
