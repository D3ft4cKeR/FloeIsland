...
export function createStopwatchModule({dock, ext}) {
    ...
    const display = new St.Label({text: '00:00.0', style_class: 'floedock-timer-display', x_align: Clutter.ActorAlign.CENTER});
    ...
    btnRow.add_child(lapBtn);
    btnRow.add_child(resetBtn);
    root.add_child(btnRow);

    // 计次列表（最多 5 条）
    const lapBox = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, x_align: Clutter.ActorAlign.CENTER, style: 'spacing: 2px;'});
    root.add_child(lapBox);
