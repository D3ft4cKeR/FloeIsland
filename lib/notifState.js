...
export class NotificationWatcher {
    constructor(ext, dock) {
        ...
        // 拦截消息托盘的 show 方法
        if (tray.show && !tray._floedockShowPatched) {
            const origShow = tray.show.bind(tray);
            tray.show = (...args) => {
                // 通知态下允许 show（显示我们的通知面板），其他情况阻止
                if (this._dock.currentState === State.NOTIFICATION)
                    return origShow(...args);
                return false;
            };
            tray._floedockShowPatched = true;
        }

        // 拦截 _showNotification
        if (tray._showNotification && !tray._floedockShowNotifPatched) {
            tray._showNotification = (...args) => false;
            tray._floedockShowNotifPatched = true;
        }
    }

    _onNotification(notification) {
        const st = this._dock.currentState;
        log(`[floedock] notif received: state=${st} title="${notification.title}"`);
        if (st === State.DOCK || st === State.NOTIFICATION) {
            log(`[floedock] notif → setState NOTIFICATION`);
...
export function createNotifSurface(dock, ext) {
    ...
    function buildCard(notification, backIndex) {
        ...
        // 顶部行：标题 + 时间
        const topRow = new St.BoxLayout({x_expand: true, style: 'spacing: 8px;'});
        ...
        const timeLabel = new St.Label({
            text: formatTime(notification.datetime),
