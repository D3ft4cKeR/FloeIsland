...
export function createNotifSurface(dock, ext) {
    ...
    function enqueue(notification) {
        // 按对象去重 + 按内容去重（避免同内容的不同实例被当两条，导致"单通知显示两份"）
        const key = `${notification.source?.title ?? ''}|${notification.title ?? ''}|${notification.body ?? ''}`;
        for (const q of queue) {
            if (q === notification) return;
            const qk = `${q.source?.title ?? ''}|${q.title ?? ''}|${q.body ?? ''}`;
            if (qk === key) return;
        }
