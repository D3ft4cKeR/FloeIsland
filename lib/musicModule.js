...
import {FloeMprisWatcher, PlaybackStatus} from './mpris.js';
...
export function createMusicModule({dock, ext}) {
    ...
    function loadCover(url) {
        ...
        const loadFromBytes = bytes => {
            try {
                ...
                logError(e, '[floeisland] decode art');
                coverPlaceholder.show();
            }
        };
    ...
    function syncPlayer() {
        ...
        // 兜底：选中的播放器处于 Stopped 且无元数据 → 视为无播放
        if (player && player.status === PlaybackStatus.STOPPED && !player.title) {
            player = null;
        }
        if (!player) {
            lastRenderKey = '';
            const names = watcher?.players.map(p => p.busName) ?? [];
            log(`[floeisland] music: no player picked; found: ${names.join(', ') || '(none)'}`);
            showEmpty();
            return;
        }
    ...
    function startPosTimer() {
        stopPosTimer();
        // 每秒无条件重同步：信号驱动之外的兜底，保证打开面板即能显示
        // 当前播放信息（MPRIS 属性缓存/信号时序问题导致的漏显示都会被纠正）
        posTimer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1000, () => {
            syncPlayer();
            return GLib.SOURCE_CONTINUE;
        });
        GLib.Source.set_name_by_id(posTimer, '[floeisland] music pos');
    }
    ...
    return {
        ...
        activate() {
            ...
            watcher = new FloeMprisWatcher({preferredBusName: preferred});
