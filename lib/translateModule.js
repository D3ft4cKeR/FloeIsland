...
export function createTranslateModule({dock, ext}) {
    ...
    root.add_child(dirBtn);

    // --- 内容区：ScrollView > vbox（可滚动） ---
    ...
    // GNOME 50：St.Entry 无 changed 信号，用 clutter_text 的 text-changed
    input.clutter_text.connect('text-changed', () => {
        generation++;
        debounceId = clearTimeoutId(debounceId);
        const q = input.text.trim();
        if (!q) {
        ...
        GLib.Source.set_name_by_id(debounceId, '[floeisland] translate debounce');
    });
