...
export async function captureFullscreen({includeCursor = true} = {}) {
    ...
    try {
        ...
        logError(e, '[floeisland] captureFullscreen');
        // 兜底：尝试交互式截图 UI（Shell.Screenshot 失败时）
        try {
            Main.screenshotUI.open().catch(() => {});
        } catch (e2) {
            logError(e2, '[floeisland] screenshot UI fallback');
        }
        return null;
    }
}
...
export function toggleAudioRecording() {
    if (_audioRec) {
        try {
            ...
            logError(e, '[floeisland] stop recording');
        }
...
export function launchApp(appId) {
    ...
    try {
        ...
        logError(e, `[floeisland] launch ${appId}`);
        return false;
    }
}
...
export function openPath(path) {
    ...
    try {
        ...
        logError(e, `[floeisland] open ${path}`);
        return false;
    }
}
...
/**
 * 打开 FloeDock 自己的设置窗口。
 * 优先用扩展实例的 openPreferences()（GNOME 45+ 标准 API，最可靠），
 * 其次按 UUID 查 Shell 扩展管理器，最后走 D-Bus。
 * 只打开本扩展的设置页，绝不触碰扩展管理器里的开关。
 * @param {object} [ext] 可选：当前扩展实例（extension.js 的 this）。
 */
export function openPrefs(ext) {
    try {
        // 1) 扩展实例自带 openPreferences（优先）
        if (ext && typeof ext.openPreferences === 'function') {
            ext.openPreferences();
            return;
        }
        // 2) 从 Shell 扩展管理器按 UUID 查找
        const manager = Main.extensionManager;
        const found = manager?.lookup?.(UUID);
        if (found && typeof found.openPreferences === 'function') {
            found.openPreferences();
            return;
        }
        // 3) 兜底：D-Bus LaunchExtensionPrefs
        ...
        logError(e, '[floeisland] open prefs');
    }
}
