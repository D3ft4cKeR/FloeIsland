...
// ---------------------------------------------------------------------------
const FloePanelLayout = GObject.registerClass(
class FloePanelLayout extends Clutter.LayoutManager {
...
// ---------------------------------------------------------------------------
const FloeTabsRow = GObject.registerClass({
    Properties: {
...
}, class FloeTabsRow extends St.Widget {
    _init(params) {
        super._init(params);
        this._tabBox = null;
...
export function createFullPanel(dock, ext) {
    ...
    const layout = new FloePanelLayout();
    ...
    const resultsLayer = new St.Widget({
        ...
        // 显式 BinLayout：让结果 ScrollView(FILL) 被约束到面板内容区，
        // 否则默认布局不约束高度 → 内容不溢出 → 无法滚动
    ...
    // 滚动条固定可交互（overlay 滚动条仅在滚动时浮现，触屏/鼠标都可用）
    ...
    function renderResults(rows, gen) {
        ...
        for (const row of rows) {
            try {
                ...
                logError(e, `[floeisland] build search row "${row.title}"`);
            }
        }
        contentArea.hide(); // 隐藏内容区，让搜索结果可点击
        resultsLayer.show();
        resultsLayer.opacity = 255;
    }
    ...
    function buildResultRow(row) {
        ...
        btn.add_child(v);
        // 单击打开结果（最可靠：搜索的目的就是点结果立即用）
        btn.connect('button-press-event', () => {
            try {
                row.run();
            } catch (e) {
                logError(e, '[floeisland] open search result');
            }
    ...
    // 统一滚轮：累积距离+平滑 ease（避免 smooth 事件一脚到底/卡顿）。
    // 兼容平滑滚动(get_scroll_delta)与离散滚轮(get_scroll_direction)。
    let scrollAccum = 0;
    function onViewportWheel(actor, event) {
        let dy = 0;
        try {
            const [, d] = event.get_scroll_delta();
            dy = d;
        } catch (e) {
            dy = 0;
        }
        ...
        scrollAccum += Math.sign(dy) * 30;
        if (Math.abs(scrollAccum) >= 48) {
            const target = clamp(adj.value + scrollAccum, adj.lower, max);
            adj.ease(target, {
                duration: 120,
    ...
    const FloeClippingLayout = GObject.registerClass(
    class FloeClippingLayout extends Clutter.LayoutManager {
        vfunc_get_preferred_width(container, forHeight) {
            let min = 0, nat = 0;
            for (const child of container.get_children()) {
        ...
        while (pagesBox.get_n_children() < modules.length) {
            ...
            if (!m.instance) {
                try {
                    ...
                    logError(e, `[floeisland] create module "${m.title}"`);
                }
            }
            const page = new St.Widget({
                ...
                width: 640,
                clip_to_allocation: true,
                layout_manager: new FloeClippingLayout(),
            });
            if (m.instance)
    ...
    function switchTo(index, {animate = true} = {}) {
        ...
        if (index !== currentIndex) {
            ...
            try {
                ...
                logError(e, `[floeisland] activate module "${modules[index]?.title}"`);
            }
    ...
    const startHoverWatch = () => {
        ...
        GLib.Source.set_name_by_id(hoverWatchId, '[floeisland] panel hover watch');
    };
    ...
    // ===================== 表面 =====================
    // 展开阶段回调：pull-start = 横拉开始（搜索栏随胶囊裁剪露出，不额外隐藏），
    // pull-done = 下拉完成（淡入 tab）。内容区不设 0——随胶囊裁剪自然显露，不割裂。
