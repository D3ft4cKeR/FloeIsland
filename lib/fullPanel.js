...
export function createFullPanel(dock, ext) {
    ...
    function buildResultRow(row) {
        ...
        btn.add_child(v);
        // 双击打开结果（GNOME 50 标准 ClickGesture，双击 n_click=2 才执行）
        const click = new Clutter.ClickGesture();
        click.connect('clicked', (gesture, actor, nClick) => {
            if (nClick && nClick % 2 === 0) {
                row.run();
                dock.setState(State.DOCK);
            }
        });
        btn.add_action(click);
    ...
    // 单次位移封顶一个滚轮格(80px)，避免平滑事件幅度大造成内容"跳跃"。
